...
describe("TaskAgentReportsPanel", () => {
  it("shows immutable role, phase, content, and provenance identity", () => {
    ...
    render(<TaskAgentReportsPanel reports={[report]} loading={false} error={null} onRefresh={onRefresh} />);
  ...
  it("renders loading, empty, and error states with the refresh lock", () => {
    const { rerender } = render(<TaskAgentReportsPanel reports={[]} loading error={null} onRefresh={vi.fn()} />);
    expect(screen.getByText("Loading reports…")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Refresh" })).toBeDisabled();
    rerender(<TaskAgentReportsPanel reports={[]} loading={false} error="offline" onRefresh={vi.fn()} />);
