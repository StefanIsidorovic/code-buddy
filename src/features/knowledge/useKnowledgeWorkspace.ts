...
export function useKnowledgeWorkspace(options: Options) {
  const { projectId, activeTranscriptId, initialization, summary, prompt, repositoryId, taskId } = options;
  ...
  const [dialogOpen, setDialogOpen] = useState(false); const [preview, setPreview] = useState<UnifiedTaskContextSelectionInfo | null>(null);
  ...
  useEffect(() => { previewRequest.current += 1; setPreview(null); setPreviewOpen(false);
    setPreviewLoading(false); setPreviewError(null); }, [projectId, initialization?.id, activeTranscriptId, taskId]);
  ...
  async function previewContext() {
    ...
    try { const value = await invokeCommand<UnifiedTaskContextSelectionInfo>("select_unified_project_task_context", { request: {
      initializationId: initialization.id, task: prompt, repositoryId, paths: [], characterBudget: 6000,
      projectId, transcriptSessionId: activeTranscriptId, taskId } });
      if (request === previewRequest.current) setPreview(value); }
    catch (reason) { if (request === previewRequest.current) { setPreview(null); setPreviewError(errorText(reason)); } }
