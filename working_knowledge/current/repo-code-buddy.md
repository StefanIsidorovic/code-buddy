...
- Frontend coverage is 257 tests across 51 files, including stale-safe secondary report loading/running, Review brief derivation/actions, delivery readiness/provenance history, and Activity presentation controls.
- Rust has 121 unit/integration tests across PTY/ACP lifecycle and model configuration, adapters, storage, synthesis, model catalog, deterministic context selection, Task persistence, complexity classification, overrides, audit history, secondary ACP isolation/orchestration, delivery readiness/provenance history, and migration.
...
- `src/App.tsx` is a 565-line composition root with no backend commands; frontend orchestration modularization is complete.
...
- ACP permissions are queued by the Rust transport and polled by `useAcpPermissions`; the state-free runtime panel renders only agent-offered choices and never auto-allows tool access.
- ACP process stderr is retained as a bounded diagnostic tail and attached to failed request exits; shared frontend presentation unwraps serialized AppError messages for readable notices.
- Registry launch separates package bootstrap from agent workspace: npx candidates use a neutral process cwd, binary candidates use project cwd, and ACP `session/new` always receives the selected repository.
...
- The Project Initialization lane now lives in `src/features/initialization/ProjectInitializationPanel.tsx`; it has a compact default setup card plus an expanded full workflow, composes the Summary card only when open, and App retains only expansion state, async commands, modal state and persistence.
...
- Session History Resume reactivates the existing transcript/Task through a dedicated stale-safe recovery hook; load replay is UI-only to avoid duplicate persistence, later events retain normal transcript affinity, a workspace switch kills a late recovered process, and recovery/lock failures now surface beside the dark-sidebar history list instead of disappearing into global errors.
...
- `src-tauri/src/delivery.rs` owns read-only Git delivery intelligence: it reports branch, HEAD, porcelain worktree status, capped changed files, HEAD provenance-note presence, and bounded recent commit provenance history under `refs/notes/provenance` without shell execution or Git writes.
- `src/features/delivery/` owns the Activity Delivery readiness hook/panel: the hook refreshes readiness plus provenance history with stale repository guards, and the panel shows clean/dirty/provenance status, recent noted/unnoted commits, and a read-only action policy without Ship/Commit/Push controls.
