...
- `src-tauri/src/delivery.rs` owns read-only Git delivery readiness inspection: it reports branch, HEAD, porcelain worktree status, capped changed files, and HEAD provenance-note presence under `refs/notes/provenance` without shell execution or Git writes.
- `src/features/delivery/` owns the Activity Delivery readiness hook/panel: the hook refreshes through `inspect_git_delivery_readiness` with stale repository guards, and the panel shows clean/dirty/provenance status plus a read-only action policy without Ship/Commit/Push controls.
