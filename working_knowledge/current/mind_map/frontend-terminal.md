...
## Watchouts
- Idle ACP drain runs once per second; tool_call_update transport noise is filtered in Rust before persistence/rendering, while meaningful initial tool calls remain visible.
- Task/Activity tabpanels use viewport-bounded desktop scrolling; context-preview dismissal never cancels or duplicates an in-flight send.
